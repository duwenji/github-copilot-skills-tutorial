...
# 第0章 Part 0: スキル形式の選択と実装 {#chapter-00-fundamentals}
...
### 0.1.1 🎓 Agent Skills はオープンスタンダード
...
#### 0.4.3.2 例2: GitHub Actions デバッグスキル（補助ファイル付き）
...
### 0.10.3 Q3. SKILL.md から JSON は自動生成できる？
...
## 0.12 SKILL.md YAML Frontmatter 完全仕様

### 0.12.1 Frontmatter とは
...
#### 0.12.2.2 `description` （必須、1024文字以下）
...
#### 0.12.3.2 `compatibility` （オプション、500文字以下）
...
#### 0.12.3.4 `allowed-tools` （オプション、実験的）
...
## 0.13 Progressive Disclosure（段階的情報開示）
...
# 第1章 Basics {#chapter-01-basics}

## 1.1 Part 1-1: Agent Skills とは何か {#section-01-basics-01-introduction}
...
### 1.1.2 Agent Skillsが解決する課題

#### 1.1.2.1 課題1: 毎回同じプロンプトを書く手間
...
### 1.1.7 Agent Skillsの活用シーン

#### 1.1.7.1 シーン1: 企業内コーディング規約の統一
...
## 1.2 Part 1-2: 従来のプロンプト手法との違い {#section-01-basics-02-vs-traditional}
...
## 1.3 Part 1-3: スキルの仕組み理解 {#section-01-basics-03-how-skills-work}
...
##### 1.3.10.4.1 戦略1: キーワード マッチング（シンプル）
...
##### 1.3.10.4.2 戦略2: セマンティック マッチング（高度）
...
# 第2章 Comparison {#chapter-02-comparison}

## 2.0 Part 2-0: 4つのLLM拡張方法の統一解説 {#section-02-comparison-00-four-concepts}
...
#### 2.0.2.2 2. Custom Instructions（カスタム指示）
...
#### 2.0.2.4 4. MCP（Model Context Protocol）
...
## 2.1 Part 2-1: MCP（Model Context Protocol）との違い {#section-02-comparison-01-vs-mcp}
...
##### 2.1.3.2.1 Agent Skills アーキテクチャ
...
##### 2.1.3.3.1 Agent Skills のデータフロー
...
#### 2.1.4.1 シーン1: チーム内のコーディング規約チェック

##### 2.1.4.1.1 Agent Skills の場合（最適）
...
#### 2.1.4.2 シーン2: リアルタイムデータベースクエリ

##### 2.1.4.2.1 Agent Skills の場合（不向き）
...
#### 2.1.4.3 シーン3: 会社固有の分析フロー（分析→改善提案→テスト生成）

##### 2.1.4.3.1 Agent Skills の場合（最適）
...
#### 2.1.4.4 シーン4: 複数の外部サービス統合（AI + DB + API）

##### 2.1.4.4.1 Agent Skills の場合（限界あり）
...
## 2.2 Part 2-2: その他の関連技術との比較 {#section-02-comparison-02-vs-other-tools}
...
### 2.2.4 Custom Instructions（カスタム指示）
...
### 2.2.5 Agent Skills と Custom Instructions の統合パターン
...
### 2.2.7 RAG (Retrieval Augmented Generation)
...
### 2.2.9 カスタム統合 (Copilot Extensions / API Integration)
...
## 2.3 Part 2-3: 4つの方法のメリット・デメリット詳細比較 {#section-02-comparison-03-pros-cons}
...
### 2.3.3 Custom Instructions（カスタム指示）
...
### 2.3.5 MCP（Model Context Protocol）
...
#### 2.3.8.1 プロンプトチェーニング との メリット・デメリット
...
##### 2.3.8.2.1 Agent Skills + RAG のメリット
...
##### 2.3.8.2.2 Agent Skills vs RAG のデメリット
...
#### 2.3.10.1 シナリオ1: スタートアップ（5-10人チーム）
...
#### 2.3.10.2 シナリオ2: 成長期スタートアップ（10-50人）
...
## 2.4 Part 2-4: 実践的なケーススタディ {#section-02-comparison-04-use-cases}
...
### 2.4.1 ケース1: Prompt を使った単発の高度な分析
...
### 2.4.2 ケース2: Custom Instructions で個人の作業効率を統一化
...
#### 2.4.2.2 Custom Instructions での解決
...
#### 2.4.2.3 なぜ Custom Instructions を選んだのか？
...
### 2.4.3 ケース3: 企業内ナレッジの体系化と共有（Agent Skills）
...
#### 2.4.3.2 Agent Skills での解決

##### 2.4.3.2.1 スキル1: 「code-style-checker」
...
##### 2.4.3.2.2 スキル2: 「sales-pitch-generator」
...
##### 2.4.3.2.3 スキル3: 「cs-troubleshooting」
...
#### 2.4.3.4 なぜ Agent Skills を選んだのか？
...
#### 2.4.4.1 ケース4-1: 金融機関 - コンプライアンスチェック

##### 2.4.4.1.1 スキル：「financial-compliance-checker」
...
#### 2.4.4.2 ケース4-2: 医療機関 - 医学診断補助

##### 2.4.4.2.1 スキル：「medical-diagnosis-assistant」
...
#### 2.4.4.3 ケース4-3: 法律事務所 - 契約書分析

##### 2.4.4.3.1 スキル：「contract-analysis」
...
#### 2.4.5.1 ケース5-1: CI/CD パイプライン設定

##### 2.4.5.1.1 スキル：「ci-cd-generator」
...
#### 2.4.5.2 ケース3-2: パフォーマンス診断

##### 2.4.5.2.1 スキル：「performance-diagnostic」
...
#### 2.4.5.3 ケース3-3: コードレビュー自動化（段階1）

##### 2.4.5.3.1 スキル：「code-review-starter」
...
### 2.4.6 ケース4: 営業・カスタマーサクセス

#### 2.4.6.1 ケース4-1: 営業提案書自動生成

##### 2.4.6.1.1 スキル：「sales-proposal-builder」
...
#### 2.4.6.2 ケース4-2: カスタマーサクセス - チャーン防止

##### 2.4.6.2.1 スキル：「churn-risk-assessment」
...
#### 2.4.7.1 ケース5-1: コンテンツカレンダー自動生成

##### 2.4.7.1.1 スキル：「content-calendar-generator」
...
#### 2.4.7.2 ケース5-2: マーケティング分析

##### 2.4.7.2.1 スキル：「marketing-campaign-analyzer」
...
### 2.4.8 ケース6: 教育 / トレーニング

#### 2.4.8.1 スキル：「employee-training-generator」
...
### 2.4.9 ケース7: セキュリティ・監査

#### 2.4.9.1 スキル：「security-audit-assistant」
...
# 第3章 Implementation {#chapter-03-implementation}

## 3.1 Part 3-1: スキル作成のステップバイステップガイド {#section-03-implementation-01-getting-started}
...
#### 3.1.5.2 ステップ3-1: スキル定義ファイルの骨組みを作成
...
#### 3.1.5.4 ステップ3-3: プロンプトテンプレートを実装
...
### 3.1.7 Phase 5: デプロイ & ドキュメント

#### 3.1.7.1 ステップ5-1: リポジトリへの登録
...
## 3.2 Part 3-2: スキルの構成要素 {#section-03-implementation-02-skill-structure}
...
## 3.3 Part 3-3: サンプルスキル #1 - コード品質分析 {#section-03-implementation-03-sample-code-analysis}
...
### 3.3.2 SKILL.md フォーマット（推奨）

#### 3.3.2.1 ファイル：`.github/skills/code-quality-analyzer/SKILL.md`
...
## 3.4 Part 3-4: サンプルスキル #2 - ドキュメント自動生成 {#section-03-implementation-04-sample-doc-generation}
...
### 3.4.2 SKILL.md フォーマット（推奨）

#### 3.4.2.1 ファイル：`.github/skills/doc-generator/SKILL.md`
...
## 3.5 Part 3-5: サンプルスキル #3 - テスト生成【応用的なスキル】 {#section-03-implementation-05-sample-test-generation}
...
### 3.5.2 SKILL.md フォーマット（推奨）

#### 3.5.2.1 ファイル：`.github/skills/test-generator/SKILL.md`
...
## 3.6 Part 3-6: スキル評価フレームワーク（evals） {#section-03-implementation-06-skill-evaluation}
...
### 3.6.2 Phase 1: テストケース設計

#### 3.6.2.1 evals.json 構造
...
### 3.6.3 Phase 2: ベースライン実行

#### 3.6.3.1 with-skill vs without-skill 比較
...
##### 3.6.3.2.2 without-skill 実行（ベースライン）
...
### 3.6.4 Phase 3: Assertions 定義

#### 3.6.4.1 Assertions とは
...
#### 3.6.4.2 Assertions 設計のコツ

##### 3.6.4.2.1 実装後に追加（最初は期待値のみでOK）
...
### 3.6.5 Phase 4: Grading（採点）

#### 3.6.5.1 grading.json 構造
...
### 3.6.6 Phase 5: 集計（Benchmarking）

#### 3.6.6.1 benchmark.json 作成
...
### 3.6.7 Phase 6: 人間によるレビュー

#### 3.6.7.1 feedback.json 作成
...
#### 3.6.10.1 Q1. Assertion で LLM 判定 vs スクリプト判定、どちらを使う？
...
# 第4章 Advanced {#chapter-04-advanced}

## 4.1 Part 4-1: チーム内スキル共有戦略 {#section-04-advanced-01-team-sharing}
...
#### 4.1.3.2 戦略2: パッケージ（npm, PyPI）での配布
...
### 4.1.5 スキル共有のベストプラクティス

#### 4.1.5.1 Practice 0: 補助リソース（スクリプト・テンプレート）の管理戦略
...
#### 4.1.5.3 Practice 2: バージョニング (Semantic Versioning)
...
### 4.1.10 インボーディング・トレーニング

#### 4.1.10.1 新人向けスキルトレーニングプログラム
...
## 4.2 Part 4-2: スキル管理とライフサイクル {#section-04-advanced-02-management}
...
#### 4.2.1.3 Stage 2: Beta（テスト・フィードバック）
...
#### 4.2.1.6 Stage 5: Deprecated（廃止予定）
...
## 4.3 Part 4-3: パフォーマンス最適化と監視 {#section-04-advanced-03-optimization}
...
### 4.3.3 パフォーマンス計測スクリプト

#### 4.3.3.1 performance-profiler.py（推奨実装）
...
### 4.3.4 プロンプト最適化による高速化

#### 4.3.4.1 テクニック1: プロンプト圧縮
...
## 4.4 Part 4-4: トラブルシューティングと問題解決 {#section-04-advanced-04-troubleshooting}
...
#### 4.4.3.5 補助スクリプト：自動検証ツール（skill-validator.py）
...
## 4.5 Part 4-5: スクリプト設計ベストプラクティス {#section-04-advanced-05-scripts-best-practices}
...
### 4.5.5 原則4：べき等性（Idempotency）

#### 4.5.5.1 エージェントは再試行する
...
#### 4.5.5.4 パターン：「create if not exists」
...
## 4.6 Part 4-6: 複数リポでの SKILL 共通化（実装ガイド） {#section-04-advanced-06-multi-repo-skill-sharing}
...
#### 4.6.2.1 submodule と subtree のコンセプト早見表
...
### 4.6.5 パターン1: Personal Skills（個人・全リポ共通）
...
#### 4.6.6.4 手順: 更新フロー（中央リポで更新 → 業務リポへ反映）
...
### 4.6.7 失敗駆動ハンズオン（submodule）

実運用で発生しやすい失敗を、意図的に再現して復旧手順を確認します。

#### 4.6.7.1 演習A: clone 後に `.github/skills` が空になる
...
#### 4.6.7.2 演習B: 共有側を更新したのに利用側で反映されない
...
```
  ...
  #### 4.6.9.4 スクリプトでの受け取り方（PowerShell 例）
...
## 4.7 Part 4-7: 運用手法（Runbook / Gate / Evidence / Scope / Rollout） {#section-04-advanced-07-operations-methodology}
...
# 第5章 Advanced Topics {#chapter-05-advanced-topics}

## 5.1 Part 5-1: 複合スキルと高度なパターン {#section-05-advanced-topics-01-composite-skills}
...
#### 5.1.1.2 複合スキル 例1: コード品質改善パイプライン
...
#### 5.1.2.3 パターン3: Conditional（条件付き実行）
...
## 5.2 Part 5-2: API統合と外部ツール連携 {#section-05-advanced-topics-02-api-integration}
...
#### 5.2.2.2 GitHub Actions ワークフロー統合
...
### 5.2.3 Slack / Teams 統合

#### 5.2.3.1 スキル結果を Slack に通知
...
## 5.3 Part 5-3: ベストプラクティスと推奨パターン {#section-05-advanced-topics-03-best-practices}
...
#### 5.3.1.1 原則1: 単一責任の原則（Single Responsibility）
...
#### 5.3.2.1 パターン1: ストレートスルー（Simple Pass-Through）
...
#### 5.3.2.2 パターン2: エンリッチメント（Enrichment）
...
#### 5.3.2.3 パターン3: アグリゲーション（Aggregation）
...
#### 5.3.2.4 パターン4: フィルタリング（Filtering）
...
### 5.3.12 スキル作成のベストプラクティス

#### 5.3.12.1 命名規則（Naming Conventions）
...
#### 5.3.12.2 簡潔さ（Conciseness）

##### 5.3.12.2.1 原則：トークンは共有資源
...
#### 5.3.12.3 モデル別テスト（Model-Specific Testing）
...
#### 5.3.12.4 Anti-Patterns（避けるべきパターン）

##### 5.3.12.4.1 Anti-Pattern 1: Windows パス
...
##### 5.3.12.4.2 Anti-Pattern 2: 過度な選択肢
...
##### 5.3.12.4.3 Anti-Pattern 3: 時間依存情報
...
##### 5.3.12.4.4 Anti-Pattern 4: 曖昧な説明
...
#### 5.3.12.6 テスト駆動開発（Evaluation-Driven Development）
